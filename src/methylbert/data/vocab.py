...
class GenomeVocab(Vocab):
    def __init__(self, k=3, max_size=None, min_freq=1, cpg=True, chg=True, chh=False):
        print("Building Vocab")
        self.kmers=k
        counter = Counter()

        # Create a look up table with 3-mers 
        import itertools
        ...
        vocabs = list(itertools.product(bases, repeat=self.kmers))
        vocabs = ["".join(elem) for elem in vocabs]
        for word in vocabs:
            counter[word] += 1
